...
@NECKS.register_module
class NonLinearNeckV1ActNN(nn.Module):
    """The non-linear neck in MoCo v2: fc-relu-fc.
    """
    ...
    def __init__(self,
                 ...
                 out_channels,
                 with_avg_pool=True):
        super(NonLinearNeckV1ActNN, self).__init__()
        self.with_avg_pool = with_avg_pool
        if with_avg_pool:
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        from actnn import QLinear, QReLU
        self.mlp = nn.Sequential(
            QLinear(in_channels, hid_channels), QReLU(),
            QLinear(hid_channels, out_channels))
...
@NECKS.register_module
class NonLinearNeckSimCLRActNN(nn.Module):
    """SimCLR non-linear neck.
    ...
    def __init__(self,
                 ...
                 with_last_bn=True,
                 with_avg_pool=True):
        super(NonLinearNeckSimCLRActNN, self).__init__()
        self.sync_bn = sync_bn
        self.with_last_bn = with_last_bn
        self.with_avg_pool = with_avg_pool
        if with_avg_pool:
        ...
        from actnn import QLinear, QReLU, QBatchNorm1d, QSyncBatchNorm

        self.relu = QReLU(inplace=False)
        self.fc0 = QLinear(in_channels, hid_channels, bias=with_bias)
        if sync_bn:
            self.bn0 = QSyncBatchNorm(hid_channels)
        else:
            self.bn0 = QBatchNorm1d(hid_channels)
        ...
        for i in range(1, num_layers):
            ...
            self.add_module(
                "fc{}".format(i),
                QLinear(hid_channels, this_channels, bias=with_bias))
            self.fc_names.append("fc{}".format(i))
            if i != num_layers - 1 or self.with_last_bn:
                if sync_bn:
                    self.add_module(
                        "bn{}".format(i),
                        QSyncBatchNorm(this_channels))
                else:
                    self.add_module(
                        "bn{}".format(i),
                        QBatchNorm1d(this_channels))
                self.bn_names.append("bn{}".format(i))
            else:
                self.bn_names.append(None)
